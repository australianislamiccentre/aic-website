...
export function middleware(request: NextRequest) {
  ...
  const csp = [
    ...
    [
      ...
      //
      // Google signals is deliberately OFF in the GA4 property (AIC runs no Google
      // Ads), so its remarketing hosts are NOT allowed anywhere in this policy:
      // country Google domains (www.google.com.au, .co.th, …), stats.g.doubleclick.net
      // and *.adtrafficquality.google. Re-enabling signals would bring back a flood
      // of CSP reports — one Sentry issue per country domain (AIC-WEBSITE-Y/X/Q…).
      'https://*.google-analytics.com',
