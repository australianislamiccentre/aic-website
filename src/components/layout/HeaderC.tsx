/**
 * Header Variant C -- Hybrid Inline + Hamburger
 *
 * Desktop: top info bar, sticky main bar with two inline dropdown groups
 *          ("What's On" and "Our Mosque") plus a hamburger icon that opens
 *          a narrow side panel (~320px) with the remaining groups ("About"
 *          and "Media & Resources"), Contact, and Donate.
 * Mobile:  ALL groups move into a full-screen hamburger drawer with
 *          accordion sections (same style as HeaderA mobile).
 *
 * Uses the shared `headerNavGroups` data from `src/data/navigation.ts`.
 *
 * @module components/layout/HeaderC
 * @see src/data/navigation.ts -- navigation data
 * @see src/components/layout/HeaderA.tsx -- useIsScrolled hook, reference patterns
 */
"use client";
...
/* ------------------------------------------------------------------ */
/*  Navigation group splits                                            */
/* ------------------------------------------------------------------ */

/** Groups displayed inline in the desktop header bar. */
const inlineGroups = [headerNavGroups[1], headerNavGroups[2]]; // What's On, Our Mosque

/** Groups displayed inside the desktop side panel. */
const sidePanelGroups = [headerNavGroups[0], headerNavGroups[3]]; // About, Media & Resources
...
const sidePanelOverlayVariants = {
  hidden: { opacity: 0 },
  visible: { opacity: 1 },
  exit: { opacity: 0 },
};
...
export function HeaderC() {
  ...
  // Lock body scroll when side panel or mobile drawer is open
  useEffect(() => {
    if (sidePanelOpen || mobileMenuOpen) {
  ...
  }, [sidePanelOpen, mobileMenuOpen]);

  // Close dropdowns and side panel on Escape
  useEffect(() => {
  ...
  const handleSidePanelNavClick = useCallback(
    (href: string) => {
      setSidePanelOpen(false);
      if (href === "/" && pathname === "/") {
        window.scrollTo({ top: 0, behavior: "smooth" });
  ...
  return (
    <>
        ...
        <div className="relative">
          <nav className="max-w-7xl mx-auto px-6">
            <div className="flex items-center justify-between h-16">
              ...
              {/* ----- Desktop inline navigation (What's On + Our Mosque) ----- */}
              <div
              ...
              <div className="flex items-center h-16">
                {/* Desktop hamburger (visible on lg+) for side panel */}
                <button
                  onClick={() => setSidePanelOpen(true)}
                  className={cn(
                    "hidden lg:flex p-2.5 rounded-lg transition-all duration-200 mx-1",
                    isScrolled
                ...
                <button
                  ...
                  className={cn(
                    "p-2.5 rounded-lg transition-all duration-200 mx-1",
                    isScrolled
                      ? "text-gray-600 hover:text-neutral-900 hover:bg-neutral-100"
                      : "text-white/90 hover:text-white hover:bg-white/10",
                  )}
      ...
      {/* ===== Desktop side panel (About + Media & Resources) ===== */}
      <AnimatePresence>
        {sidePanelOpen && (
          <>
            {/* Backdrop */}
            <motion.div
              variants={sidePanelOverlayVariants}
              initial="hidden"
              animate="visible"
              exit="exit"
              transition={{ duration: 0.2 }}
              className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 hidden lg:block"
            ...
            <motion.div
              ...
              className="fixed top-0 right-0 h-full w-[320px] bg-neutral-900 z-50 hidden lg:flex flex-col shadow-2xl"
            >
              {/* Panel header */}
              ...
              <div className="flex-1 overflow-y-auto px-6 py-6">
                {sidePanelGroups.map((group) => (
                  <div key={group.label} className="mb-8">
                    <h3 className="text-xs font-semibold uppercase tracking-wider text-white/50 mb-3 px-2">
                      {group.label}
                    </h3>
                      ...
                      {group.links.map((link) =>
                        link.external ? (
                          <a
                            ...
                            onClick={() => setSidePanelOpen(false)}
                            className={cn(
                              "flex items-center justify-between px-3 py-2.5 rounded-lg text-sm text-white/80 hover:text-white hover:bg-white/10 transition-colors",
                          ...
                          <Link
                            key={link.href}
                            href={link.href}
                            onClick={() => handleSidePanelNavClick(link.href)}
                            className={cn(
                              "block px-3 py-2.5 rounded-lg text-sm text-white/80 hover:text-white hover:bg-white/10 transition-colors",
                              isActive(link.href) && "text-lime-400",
                            )}
                          >
                            {link.name}
                ...
                <div className="border-t border-white/10 pt-6 space-y-1">
                  <Link
                    href="/contact"
                    onClick={() => handleSidePanelNavClick("/contact")}
                    className={cn(
                      "block px-3 py-2.5 rounded-lg text-sm font-semibold text-white/80 hover:text-white hover:bg-white/10 transition-colors",
                  ...
                  {/* Donate link with Heart icon */}
                  <Link
                    href="/donate"
                    onClick={() => handleSidePanelNavClick("/donate")}
                    className="flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm font-semibold text-amber-400 hover:text-amber-300 hover:bg-white/10 transition-colors"
                  >
                    <Heart className="w-4 h-4" />
                    Donate
                  </Link>
      ...
      {/* ===== Mobile drawer (full-screen, all groups) ===== */}
      <AnimatePresence>
        {mobileMenuOpen && (
          <>
            {/* Backdrop */}
