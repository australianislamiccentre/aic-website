...
`eventType` field (radio): `"single"` | `"multi"` | `"recurring"`. GROQ date comparisons take a `$today` parameter (Melbourne calendar date from `getMelbourneDateString()`), never `now()` — see "Dates and hydration" Rule 4. The Studio desk lists use the same filters via `src/sanity/deskFilters.ts`.
...
Only `datetime` fields (full UTC ISO timestamps) may be compared against `now()` — that comparison is tz-neutral because both sides are absolute instants. Check the schema before assuming: `announcement.expiresAt` is a `date` field, so it uses `expiresAt >= $today` like the event dates. Comparing a `"YYYY-MM-DD"` string against `now()` is doubly wrong — the date string sorts before the same day's timestamp, so the item disappears at 10–11am Melbourne on its last day. No production query uses `now()` today; `src/sanity/lib/queries.test.ts` runs the announcement queries through `groq-js` to keep it that way.
...
9. **GROQ `now()` created a 10-hour Melbourne-midnight skew on event auto-hide (2026-04-17):** Event/program queries used `string::split(string(now()), "T")[0]` to extract "today's date" for filtering. `now()` returns UTC, so between Melbourne midnight (00:00 AEST/AEDT) and UTC midnight — roughly 10 hours each day — the extracted date was *yesterday* in Melbourne terms. An admin setting an event to end on April 19 would see it keep showing until 10am on April 20 (= UTC midnight). **Fix:** Changed all date-filtering queries (`eventsQuery`, `featuredEventsQuery`, `programsQuery`) to accept a `$today` parameter. Fetch functions compute `today = getMelbourneDateString()` server-side and pass it in. The announcement queries were left on `expiresAt > now()` on the mistaken belief that `expiresAt` was a `datetime`; it is a `date`, and correction #10 fixed it. New rule in "Dates and hydration" documents this split.

10. **Sanity settings that looked wired but weren't (2026-09-15 audit):** The client reported the event "form embed" not working. `siteSettings.allowedEmbedDomains` listed only `form.jotform.com`, while JotForm serves any form with a payment field from `pci.jotform.com` — so `FormEmbed.tsx` silently rendered nothing for every paid registration (the CSP already allowed `*.jotform.com`, so the two allowlists disagreed). The iframe was also server-rendered, so JotForm's one `setHeight` message arrived before hydration and the form stayed in a 320px box on slower devices. A full end-to-end audit of every schema found the same class of problem elsewhere: `worshippersPageSettings.khutbahVisible` fetched but never read; Visit icons keyed by lowercase names while the schema stored PascalCase; the imams query omitting seven fields the page renders; `announcement.expiresAt` (a `date`) compared with `now()`; Studio "Expired" desk list negating a null-producing GROQ expression; the event-inquiry API taking its recipient address from the browser; two legacy draft-mode endpoints that could be enabled by anyone; every page title doubling the site name; JSON-LD carrying a hardcoded old phone number. **Fix:** (1) `src/lib/embed-domains.ts` is the single source of truth for trusted iframe domains, shared by the CSP, the event page and a Studio validation warning. (2) `src/sanity/deskFilters.ts` holds the desk filters, tested with `groq-js` against the site's own queries. (3) `src/lib/seo.ts` `pageTitle()` strips a typed site name; the Studio help text no longer suggests typing it. (4) `src/lib/structured-data.ts` builds JSON-LD from Site Settings. (5) The revalidate webhook uses `revalidateTag(tag, { expire: 0 })` — a profile string only marks entries stale in Next 16, so the first visit after publishing still saw old content. The audit's remaining open findings are listed in the PR description for the branch `fix/sanity-form-embeds-audit`. The lesson: "fetched and typed" is not "rendered" — a field is only done when a test proves the Studio value reaches the DOM, and any list a component keys on (icons, variants, domains) must accept exactly what the schema's option list stores.
